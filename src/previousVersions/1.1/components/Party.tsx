    ...
    const spawnCircles = () => {
      ...
      for (let col = 0; col < numColumns; col++) {
        for (let row = 0; row < numRows; row++) {
          ...
          const rowFactor =
            (Math.pow(row - centerRow, 2) * sphereRadius) / maxRowFactor;
          ...
          circle.style.backgroundColor = getRainbowColor(
            col + numColumns,
            numColumns
          );
          circle.style.top = `calc(${
            row * rowHeight * sphereHeightScale +
            (100 - sphereHeightScale * 100)
          }dvh - ${ballRadius * ((row + (row > 0 ? 1 : 0)) / numRows)}px)`;
          ...
          const existingRule = [...document.styleSheets[0].cssRules].some(
            (rule) =>
              rule instanceof CSSKeyframesRule && rule.name === keyframeName
          );
          ...
          if (!existingRule) {
            const keyframes = `
              @keyframes ${keyframeName} {
                0% { left: calc(50vw - ${
                  ballRadius / 2
                }px); z-index: 1000; animation-timing-function: ease-out; opacity: 1; }
                25% { left: calc(${
                  100 - rowFactor
                }vw - ${ballRadius}px); z-index: 500; animation-timing-function: ease-in; }
                50% { left: calc(50vw - ${
                  ballRadius / 2
                }px); z-index: 1; animation-timing-function: ease-out; }
